...
KALI_CONFIG_OPTS="--distribution $KALI_DIST -- --variant $KALI_VARIANT --linux-packages linux-image-3.18.0-kali3"
if [ -n "$OPT_pu" ]; then
...
for KALI_ARCH in $KALI_ARCHES; do
	...
	[ $? -eq 0 ] || failure
	run_and_log lb config -a $KALI_ARCH $KALI_CONFIG_OPTS --bootappend-live "boot=live config hostname=drop user=hitmani" "$@"
	[ $? -eq 0 ] || failure
